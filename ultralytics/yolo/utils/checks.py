...
def check_version(current: str = "0.0.0",
    ...
    try:
        ...
        # Fallback to distutils if packaging is not available
...
def check_requirements(requirements=ROOT.parent / 'requirements.txt', exclude=(), install=True, cmds=''):
    ...
    if isinstance(requirements, Path):  # requirements.txt file
        ...
        with file.open() as f:
            # Parse requirements.txt manually since pkg_resources may not be available
            requirements = []
            for line in f:
                line = line.strip()
    ...
    for r in requirements:
        try:
            ...
            pkg_name = re.match(r'([a-zA-Z0-9_-]+)', r).group(1) if re.match(r'([a-zA-Z0-9_-]+)', r) else r
            # Check if package is installed
            version(pkg_name)
        except PackageNotFoundError:  # exception if requirements not met
            s += f'"{r}" '
            n += 1
